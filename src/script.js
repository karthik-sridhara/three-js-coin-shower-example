...
function createCoin(position,velocity=undefined){
...
const debugObject = {
    ...
    coinShower: () => {
        for (let i = 0; i < debugObject.numberOfCoins; i++){
            createCoin(
                {
                    x:(Math.random()) *0.4,
                    y:0,
...
gui.add(debugObject, "numberOfCoins").min(1).max(100).step(1).name("Number of Coins");
gui.add(debugObject, "coinShower").name("Coin Shower");